...
import { buildKakaoMapUrl, buildNaverSearchUrl } from '../../lib/replacement-candidates';
...
export default function PlaceDetailScreen() {
  ...
  if (!name || !kakaoPlaceId) {
    return <SafeAreaView style={s.safe}><BackBar /><View style={s.content}><Text style={s.error}>{t('modeFlow.courseResult.loadError')}</Text></View></SafeAreaView>;
  }
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      <View style={s.content}>
        ...
        <TouchableOpacity accessibilityRole="link" onPress={() => void WebBrowser.openBrowserAsync(buildNaverSearchUrl(name))} style={s.primary}><Text style={s.primaryText}>{t('modeFlow.courseResult.naverReviews')}</Text></TouchableOpacity>
