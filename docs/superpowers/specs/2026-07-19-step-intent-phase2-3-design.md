...
|---|---|
| Phase 4 | 연기 — 문서화만 |
| AI 파서 호출 조건 | `additionalRequest` 비어있지 않음 **AND** 규칙 파서 미검출/저신뢰일 때만. 사전 히트·자유텍스트 없음 = AI 0건 |
...
**저신뢰 판정(규칙)**: `additionalRequest`에 의미 있는 잔여 텍스트가 있는데 매칭 intent가 0이거나, 부정 마커(`말고`/`not`/`except`)가 있는데 규칙이 부정 처리를 못한 경우. 구체 임계값은 플랜에서 확정.
