...
describe('mode별 FeelingInput 빌더', () => {
  ...
  it('location 공백/미입력은 undefined로 정규화', () => {
    ...
    expect(buildCourseInput({ idea: '한강', budget: '', duration: '', location: '  ' }).location).toBeUndefined();
  });
  ...
  it('coords 미지정 시 undefined', () => {
    expect(buildFeelingInput({ mood: 'quiet', budget: 'low', duration: '1h' }).coords).toBeUndefined();
