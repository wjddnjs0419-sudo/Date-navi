-- latest_request.lockedSteps is redundant with recommendation_course_steps.locked (the
-- authoritative per-step lock state, which the client rebuilds outbound before every
-- recommend-date call). The mutation RPC used to persist it in an abbreviated
-- {stepId,candidateId,kakaoPlaceId} shape that no longer satisfies recommendationRequestSchema,
-- so every reader of latest_request (client hydration + the replacement-candidates edge)
-- rejected any session that had a locked step. Stop persisting lockedSteps entirely and
-- backfill existing rows so the field disappears everywhere.
...
begin
  ...
  update public.recommendation_sessions set
    ...
    latest_request = jsonb_set(coalesce(v_request, latest_request, original_request), '{courseSteps}', v_request_steps) - 'lockedSteps',
    updated_at = now()
