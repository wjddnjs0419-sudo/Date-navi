...
- [ ] 인사이트 게시판 상세 라벨/표지 비율 점검
  - 증상: 게시판 명을 `인사이트`로 변경했지만 상세 페이지에서 여전히 `미디어`로 표시됨
  - 점검: 상세 헤더/브레드크럼/메타 라벨 하드코딩 위치 확인 후 명칭 일관화
  - 점검: 표지 이미지 업로드 프리뷰 및 상세 렌더링의 현재 비율 확인, `1:1` 정사각형 업로드와 표시 정책 정리
  - 보류 사유: 현재 작업 폴더에서는 해당 게시판 UI 코드를 아직 특정하지 못해 대상 프로젝트 확인 필요
