...
// 계약: props 시그니처 { visible, message, onHide } 와 동작(메시지 노출 + 버튼 닫힘)을 고정한다.
// 이 계약은 세 컨슈머(share/send, mode-flow/course-result, card/confirm)가 의존하므로 리스타일이 깨선 안 된다.
// 2026-07-21: 목업대로 '버튼 닫힘'으로 확정 — 자동닫힘 타이머 없음.
describe('SuccessModal 계약', () => {
  ...
  it('자동으로 닫히지 않고, 확인 버튼(onPress)으로만 onHide 를 호출한다', () => {
    jest.useFakeTimers();
    ...
    // 시간이 지나도 자동으로 닫히지 않는다.
    TR.act(() => { jest.advanceTimersByTime(5000); });
    expect(onHide).not.toHaveBeenCalled();
    // 확인 버튼을 누르면 onHide 가 호출된다.
    const pressables = tree.root
      .findAllByType(TouchableOpacity)
      .filter((n) => typeof n.props.onPress === 'function');
    expect(pressables.length).toBeGreaterThan(0);
    TR.act(() => { pressables[pressables.length - 1].props.onPress(); });
