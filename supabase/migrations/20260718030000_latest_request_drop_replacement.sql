-- After a replace mutation, the attested request (which carries the one-shot
-- `replacement` field) was stored verbatim as latest_request. Subsequent add /
-- unlocked-regenerate calls spread latest_request client-side, so the stale
-- `replacement` pushed recommend-date into its replacement branch and failed
-- validation with 422 COURSE_VALIDATION_FAILED. latest_request must represent
-- standing request state only: strip one-shot fields at persist time and
-- repair sessions that were already polluted.
begin;
...
update public.recommendation_sessions
  set latest_request = latest_request - 'replacement'
  where latest_request ? 'replacement';
