-- A replace mutation was rejected with constraint_violation whenever the fresh search that
-- verified the chosen replacement happened to reuse the target step's ephemeral candidate number
-- (candidateId restarts at candidate_001 on every search, so e.g. replacing the first step of a
-- course almost always collides). The "replacement must actually change the step" guard now
-- compares the stable Kakao place ID instead of the per-search candidateId.
