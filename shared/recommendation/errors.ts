...
export const RECOMMENDATION_ERROR_METADATA: Record<RecommendationErrorCode, Omit<RecommendationError, 'code'>> = {
  ...
  AI_BURST_LIMITED: { messages: { ko: '짧은 시간에 생성 요청이 많아요. 잠시 후 다시 시도해 주세요.', en: 'Too many course requests in a short time. Please try again shortly.' }, retryable: true, requiresConditionEdit: false },
  AI_DAILY_LIMITED: { messages: { ko: '오늘의 코스 생성 횟수를 모두 사용했어요. 자정 이후 다시 시도해 주세요.', en: 'You have used today’s course generations. Please try again after midnight.' }, retryable: true, requiresConditionEdit: false },
...
export const recommendationErrorSchema = z.object({
  code: z.enum(['LOCATION_REQUIRED', 'INVALID_INPUT', 'PLACE_SEARCH_TIMEOUT', 'PLACE_SEARCH_RATE_LIMITED', 'INSUFFICIENT_CANDIDATES', 'STEP_INTENT_UNSATISFIED', 'STEP_PIN_UNAVAILABLE', 'AI_TIMEOUT', 'AI_INVALID_RESPONSE', 'AI_REQUEST_ALREADY_RUNNING', 'AI_BURST_LIMITED', 'AI_DAILY_LIMITED', 'AI_LIMIT_UNAVAILABLE', 'COURSE_VALIDATION_FAILED', 'AUTH_EXPIRED', 'NETWORK_ERROR', 'UNKNOWN']),
