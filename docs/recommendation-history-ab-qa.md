...
**2026-07-26 07:28 ~ 07:48 (UTC) 세션은 전부 수동 QA 데이터이므로 분석에서 제외한다.**
같은 좌표에서 수 초 간격으로 연속 생성한 인위적 표본이고, 후반부는 mode를 잠시
`treatment`로 강제해 **control로 배정된 커플이 treatment로 기록**됐다. 그대로 넣으면
`sameAreaRepeatRate`가 0으로 과대평가되고 arm 배정도 오염된다.

QA 결과 자체는 기능 검증용으로 유효했다. 동일 계정·동일 좌표·동일 조건에서
control 2회는 같은 장소(가마솥김치전골·아트티나)를 반복했고, treatment 2회는
제외 수 2 → 4로 늘며 매번 새로운 장소를 냈다.
