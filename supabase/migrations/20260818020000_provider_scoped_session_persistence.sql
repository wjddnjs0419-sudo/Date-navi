...
-- Legacy rows and Kakao writers keep their columns. Naver rows deliberately
-- have no fabricated Kakao ID.
alter table public.recommendation_course_steps
  alter column original_kakao_place_id drop not null,
  alter column current_kakao_place_id drop not null;
...
declare v_owner uuid := auth.uid(); v_attestation public.recommendation_generation_attestations%rowtype; v_couple_id text; v_request jsonb; v_response jsonb; v_session_id text; v_step jsonb; v_provider text; v_provider_place_id text;
begin
  ...
  for v_step in select value from jsonb_array_elements(v_response #> '{course,steps}') loop
    v_provider := coalesce(v_step #>> '{placeIdentity,provider}', case when v_step ? 'kakaoPlaceId' then 'kakao' end); v_provider_place_id := coalesce(v_step #>> '{placeIdentity,providerPlaceId}',v_step->>'kakaoPlaceId');
    ...
    insert into public.recommendation_course_steps (session_id,step_id,step_order,category,label,original_candidate_id,original_kakao_place_id,original_place_provider,original_provider_place_id,current_candidate_id,current_kakao_place_id,current_place_provider,current_provider_place_id,place_name,address,road_address,map_url,latitude,longitude,reason,locked) values (v_session_id,v_step->>'stepId',(v_step->>'order')::smallint,v_step->>'category',v_step->>'label',v_step->>'candidateId',case when v_provider='kakao' then v_provider_place_id end,v_provider,v_provider_place_id,v_step->>'candidateId',case when v_provider='kakao' then v_provider_place_id end,v_provider,v_provider_place_id,v_step->>'name',coalesce(v_step->>'address',''),coalesce(v_step->>'roadAddress',''),coalesce(v_step->>'mapUrl',''),(v_step->>'latitude')::double precision,(v_step->>'longitude')::double precision,v_step->>'reason',coalesce((v_step->>'locked')::boolean,false));
  end loop;
