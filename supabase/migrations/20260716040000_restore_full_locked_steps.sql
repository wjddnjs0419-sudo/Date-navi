-- The original bug (session AP) was that apply_recommendation_session_mutation persisted
-- latest_request.lockedSteps in an abbreviated {stepId,candidateId,kakaoPlaceId} form that no
-- longer satisfies lockedCourseStepInputSchema (which needs the place facts). Migration
-- 20260716020000 wrongly "fixed" this by stripping lockedSteps entirely, which breaks
-- validateRecommendDateResponseForRequest (it uses lockedSteps to verify each step's locked
-- flag). Restore lockedSteps persistence with the FULL 9-field place-fact shape shipped clients
-- send, and backfill existing sessions the same way. locked flag is derived from membership, so
-- the field itself is not persisted.
...
-- Backfill: rebuild latest_request.lockedSteps for existing sessions from the row columns, using
-- the full 9-field place-fact shape (locked steps only). Sessions with no locked steps get the key
-- removed. extra_float_digits=3 keeps coordinates at full precision to match get_recommendation_session.
set local extra_float_digits = 3;
update public.recommendation_sessions s
  set latest_request = case
    when (select count(*) from public.recommendation_course_steps cs where cs.session_id = s.id and cs.locked) = 0
      then coalesce(s.latest_request, s.original_request) - 'lockedSteps'
      else jsonb_set(
        coalesce(s.latest_request, s.original_request),
        '{lockedSteps}',
        (select jsonb_agg(jsonb_build_object(
           'stepId', cs.step_id, 'candidateId', cs.current_candidate_id, 'kakaoPlaceId', cs.current_kakao_place_id,
           'name', cs.place_name, 'address', cs.address, 'roadAddress', cs.road_address, 'mapUrl', cs.map_url,
           'latitude', cs.latitude, 'longitude', cs.longitude
         ) order by cs.step_order)
         from public.recommendation_course_steps cs where cs.session_id = s.id and cs.locked)
      )
  end
  where s.latest_request is not null;
