-- Confirmed date_cards are read by both partners of a couple, who may run the app in
-- different languages. The recommendation pipeline now emits card texts in ko AND en
-- (cards[0].i18n); persist that block so each viewer reads the card in their own language.
